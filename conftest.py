...
def pw_page(
    ...
    # Move video after context is closed (only if test failed and not in CI)
    if video_obj and base_filename:
        try:
            ...
            if video_path and Path(video_path).exists():
                ...
                logging.getLogger(__name__).info("Video saved: %s", video_dest)
        except Exception as e:
            logging.getLogger(__name__).warning("Failed to save video: %s", e)
