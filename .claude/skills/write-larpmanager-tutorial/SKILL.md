...
The existing tutorials (see `screenshots.md` for the full list with their order) cover: create-organization, player-information, organization-roles, organization-appearance, manage-events, event-roles, event-appearance, promotion, registrations, manage-ticket, registration-form, registration-accounting, payments, characters, character-references, pdf-generation, character-form, character-creation, character-inventory, xp, factions, ensemble, guilds, plots, writing-extras, matchmaker, casting, quests, deadlines, users, user-flags, membership, accounting, checkin, debrief, event-tools, organization-tools, italian-associations, manage-mails.
...
- **Believable demo data**: one fictional setting across all tutorials (organization "Silver Lantern", event "The Ember Crown", characters Aldric Vane, Mira Solen, ...; players Elena Ricci, Marco Bellini, ...; `@example.com` emails, the standard example IBAN), never "Test Teeeest", lorem ipsum, real people or real accounts. Use the same names in the tutorial text examples.
