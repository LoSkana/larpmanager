...
class ExeConfigForm(MyForm):
    ...
    def get_config_fields(self):
        ...
        ls.append(("interface", "admin_links", 2, section, label, help_text))
    ...
    def get_config_others(self, ls):
        if "custom_mail" in self.params["features"]:
            ...
            label = _("Use TLD")
            ls.append(("mail_server", "use_tls", 2, section, label, help_text))

            label = _("Host Address")
            ls.append(("mail_server", "host", 1, section, label, help_text))

            label = _("Port")
            ls.append(("mail_server", "port", 4, section, label, help_text))

            label = _("Username of account")
            ls.append(("mail_server", "host_user", 1, section, label, help_text))

            label = _("Password of account")
            ls.append(("mail_server", "host_password", 1, section, label, help_text))
        ...
        if "centauri" in self.params["features"]:
            ...
            ls.append(("centauri", "prob", 4, section, label, help_text))

            label = _("Badge")
            help_text = _("Name of badge to be awarded")
            ls.append(("centauri", "badge", 1, section, label, help_text))

            label = _("Description")
            help_text = _("Description to be shown on the special page")
            ls.append(("centauri", "descr", 1, section, label, help_text))

            label = _("Page")
            help_text = _("Contents of the special page")
            ls.append(("centauri", "content", 3, section, label, help_text))
    ...
    def get_config_members(self, ls):
        if "deadlines" in self.params["features"]:
            ...
            ls.append(("deadline", "tolerance", 4, section, label, help_text))

            label = _("Frequency")
            help_text = _("Sets how often reminder emails are sent, in days (if not set, no emails are sent)")
            ls.append(("deadline", "days", 4, section, label, help_text))
        ...
        if "membership" in self.params["features"]:
            ...
            ls.append(("membership", "grazing", 4, section, label, help_text))

            label = _("Tax code check")
            help_text = _("Apply a check on the tax code of players")
            ls.append(("membership", "cf", 2, section, label, help_text))
        ...
        if "vote" in self.params["features"]:
            ...
            ls.append(("vote", "open", 2, section, label, help_text))

            label = _("Candidates")
            help_text = _("Candidates at the polls")
            ls.append(("vote", "candidates", 6, section, label, help_text, self.instance.id))

            label = _("Minimum votes")
            help_text = _("Minimum number of votes")
            ls.append(("vote", "min", 4, section, label, help_text))

            label = _("Maximum votes")
            help_text = _("Maximum number of votes")
            ls.append(("vote", "max", 4, section, label, help_text))
    ...
    def get_config_accounting(self, ls):
        if "payment" in self.params["features"]:
            ...
            ls.append(("payment", "hide_amount", 2, section, label, help_text))

            label = _("Unique code")
            help_text = _("If checked: Adds a unique code to each payment, which helps in being able to recognize it")
            ls.append(("payment", "special_code", 2, section, label, help_text))
        ...
        if "organization_tax" in self.params["features"]:
            ...
            ls.append(("organization_tax", "perc", 4, section, label, help_text))
        ...
        if "payment_fees" in self.params["features"]:
            ...
            ls.append(("payment_fees", "user", 2, section, label, help_text))
    ...
    def get_config_einvoice(self, ls):
        ...
        label = _("Name")
        help_text = ""
        ls.append(("einvoice", "denominazione", 1, section, label, help_text))

        label = _("Fiscal code")
        help_text = ""
        ls.append(("einvoice", "idcodice", 1, section, label, help_text))

        label = _("VAT No.")
        help_text = ""
        ls.append(("einvoice", "partitaiva", 1, section, label, help_text))

        label = _("Tax regime")
        help_text = "RF19: forfettario, RF01: ordinario, RF05: agevolato, RF07: commerciale"
        ls.append(("einvoice", "regimefiscale", 1, section, label, help_text))

        label = _("VAT rate")
        help_text = _("If absent, indicate 0")
        ls.append(("einvoice", "aliquotaiva", 1, section, label, help_text))

        label = _("Nature")
        help_text = _("Indicate only if rate 0")
        ls.append(("einvoice", "natura", 1, section, label, help_text))

        label = _("Address")
        help_text = ""
        ls.append(("einvoice", "indirizzo", 1, section, label, help_text))

        label = _("House number")
        help_text = ""
        ls.append(("einvoice", "numerocivico", 1, section, label, help_text))

        label = _("Cap")
        help_text = ""
        ls.append(("einvoice", "cap", 1, section, label, help_text))

        label = _("Municipality")
        help_text = ""
        ls.append(("einvoice", "comune", 1, section, label, help_text))

        label = _("Province")
        help_text = _("Code two capital letters")
        ls.append(("einvoice", "provincia", 1, section, label, help_text))

        label = _("Nation")
        help_text = _("Code two capital letters")
        ls.append(("einvoice", "nazione", 1, section, label, help_text))

        label = _("Recipient Code")
        help_text = _("Intermediary channel code")
        ls.append(("einvoice", "codicedestinatario", 1, section, label, help_text))
