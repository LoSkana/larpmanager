...
class OrgaEventTextForm(MyForm):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not get_event_config(self.params["event"].id, "user_character_approval", default_value=False, context=self.params):
...
class OrgaRunForm(ConfigForm):
    ...
    def set_configs(self) -> None:  # noqa: C901 - Complex form configuration with feature-dependent field setup
        ...
        if not get_event_config(self.params["event"].id, "writing_field_visibility", default_value=False, context=self.params):
            return None
...
class OrgaPreferencesForm(ExePreferencesForm):
    ...
    def add_writing_configs(self, basics: set, event_id: int, help_text: str, writing_section: tuple) -> None:
        ...
        if writing_section[0] == "character":
            ...
            if get_event_config(self.params["event"].id, "user_character_approval", default_value=False, context=self.params):
