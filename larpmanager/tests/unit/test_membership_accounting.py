...
class TestMembershipCreation(BaseTestCase):
    ...
    def test_create_basic_membership(self):
        ...
        Membership.objects.filter(member=member, assoc=assoc).delete()

        membership = Membership(member=member, assoc=assoc, credit=Decimal("0.00"), tokens=Decimal("0.00"))
        membership.save()

        self.assertIsNotNone(membership.id)
        self.assertEqual(membership.member, member)
        self.assertEqual(membership.assoc, assoc)
    ...
    def test_create_membership_with_initial_tokens(self):
        ...
        membership = Membership(member=member, assoc=assoc, credit=Decimal("0.00"), tokens=Decimal("10.00"))
        membership.save()
...
class TestMembershipAccountingAutomation(BaseTestCase):
    ...
    def test_tokens_automatic_update_on_token_given(self, mock_features):
        ...
        AccountingItemOther.objects.filter(member=member, assoc=assoc, oth=OtherChoices.TOKEN).delete()
        AccountingItemPayment.objects.filter(member=member, assoc=assoc, pay=PaymentChoices.TOKEN).delete()
        ...
        AccountingItemOther.objects.create(
            member=member, assoc=assoc, oth=OtherChoices.TOKEN, value=Decimal("5.00"), descr="Test tokens"
        )
    ...
    def test_tokens_automatic_update_on_token_used(self, mock_features):
        ...
        # Give tokens first
        AccountingItemOther.objects.create(
            member=member, assoc=assoc, oth=OtherChoices.TOKEN, value=Decimal("10.00"), descr="Test tokens"
        )

        # Use tokens
        payment = AccountingItemPayment.objects.create(
            member=member, assoc=assoc, reg=registration, pay=PaymentChoices.TOKEN, value=Decimal("3.00")
        )
    ...
    def test_credit_automatic_update_on_credit_given(self, mock_features):
        ...
        AccountingItemOther.objects.filter(member=member, assoc=assoc, oth__in=[OtherChoices.CREDIT, OtherChoices.REFUND]).delete()
        AccountingItemPayment.objects.filter(member=member, assoc=assoc, pay=PaymentChoices.CREDIT).delete()
        AccountingItemExpense.objects.filter(member=member, assoc=assoc).delete()
        ...
        # Give credit
        AccountingItemOther.objects.create(
            member=member, assoc=assoc, oth=OtherChoices.CREDIT, value=Decimal("50.00"), descr="Test credit"
        )
    ...
    def test_credit_automatic_update_on_credit_used(self, mock_features):
        ...
        # Give credit first
        AccountingItemOther.objects.create(
            member=member, assoc=assoc, oth=OtherChoices.CREDIT, value=Decimal("100.00"), descr="Test credit"
        )

        # Use credit
        payment = AccountingItemPayment.objects.create(
            member=member, assoc=assoc, reg=registration, pay=PaymentChoices.CREDIT, value=Decimal("25.00")
        )
        # Trigger signal by saving again (signal only fires on update, not create)
        payment.save()
    ...
    def test_credit_automatic_update_on_expense_approved(self, mock_features):
        ...
        AccountingItemExpense.objects.create(
            member=member, assoc=assoc, value=Decimal("30.00"), descr="Test expense", is_approved=True
        )
    ...
    def test_credit_automatic_update_on_refund(self, mock_features):
        ...
        AccountingItemOther.objects.create(
            member=member, assoc=assoc, run=run, oth=OtherChoices.REFUND, value=Decimal("20.00"), descr="Test refund"
        )
    ...
    def test_multiple_token_operations(self, mock_features):
        ...
        AccountingItemOther.objects.create(
            member=member, assoc=assoc, oth=OtherChoices.TOKEN, value=Decimal("5.00"), descr="Tokens 1"
        )
        AccountingItemOther.objects.create(
            member=member, assoc=assoc, oth=OtherChoices.TOKEN, value=Decimal("3.00"), descr="Tokens 2"
        )

        # Use some tokens
        payment = AccountingItemPayment.objects.create(
            member=member, assoc=assoc, reg=registration, pay=PaymentChoices.TOKEN, value=Decimal("2.00")
        )
        # Trigger signal by saving again (signal only fires on update, not create)
        payment.save()
    ...
    def test_multiple_credit_operations(self, mock_features):
        ...
        # Give credit
        AccountingItemOther.objects.create(
            member=member, assoc=assoc, oth=OtherChoices.CREDIT, value=Decimal("100.00"), descr="Credit 1"
        )

        # Add expense
        AccountingItemExpense.objects.create(
            member=member, assoc=assoc, value=Decimal("50.00"), descr="Expense", is_approved=True
        )

        # Use some credit
        payment = AccountingItemPayment.objects.create(
            member=member, assoc=assoc, reg=registration, pay=PaymentChoices.CREDIT, value=Decimal("30.00")
        )
        # Trigger signal by saving again (signal only fires on update, not create)
        payment.save()
    ...
    def test_token_balance_never_negative(self, mock_features):
        ...
        payment = AccountingItemPayment.objects.create(
            member=member, assoc=assoc, reg=registration, pay=PaymentChoices.TOKEN, value=Decimal("5.00")
        )
        # Trigger signal by saving again (signal only fires on update, not create)
        payment.save()
