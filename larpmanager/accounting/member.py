...
def info_accounting(request, ctx):
    ...
    ctx["reg_list"] = []
    # check if this year membership fee has been payed
    ...
    # get donations
    if "donate" in request.assoc["features"]:
        ctx["donations"] = AccountingItemDonation.objects.filter(member=member, assoc_id=ctx["a_id"]).order_by(
            "-created"
        )

    # get collection
    if "collection" in request.assoc["features"]:
        ctx["collections"] = Collection.objects.filter(organizer=member, assoc_id=ctx["a_id"])
        ctx["collection_gifts"] = AccountingItemCollection.objects.filter(
            member=member, collection__assoc_id=ctx["a_id"]
        )

        # check if they need to pay the membership fee right now
    ctx["reg_years"] = {}
    ...
    reg_que = Registration.objects.filter(member=member, run__event__assoc_id=ctx["a_id"]).exclude(
        run__development__in=[Run.CANC]
    )
