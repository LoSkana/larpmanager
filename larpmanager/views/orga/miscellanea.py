...
from larpmanager.utils.core.common import get_album_cod, get_element, get_elements, get_object_uuid
...
def orga_warehouse_area(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    context["list"] = get_elements(context["event"].id, WarehouseArea)
...
def orga_warehouse_area_assignments(request: HttpRequest, event_slug: str, area_uuid: str) -> HttpResponse:
    ...
    for el in get_elements(context["event"].id, WarehouseItemAssignment).filter(event=context["event"]):
...
def orga_warehouse_checks(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    for el in get_elements(context["event"].id, WarehouseItemAssignment).select_related("area", "item"):
...
def orga_warehouse_manifest(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    for el in get_elements(context["event"].id, WarehouseItemAssignment).select_related(
...
def orga_warehouse_commit_preview(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    for el in get_elements(context["event"].id, WarehouseItemAssignment).filter(event=context["event"], loaded=True):
        if el.quantity:
