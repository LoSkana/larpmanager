...
def orga_exp_systems(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    context["list"] = get_elements(context["event"].id, SystemExp).order_by("order")
...
def orga_exp_deliveries(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    deliveries = list(get_elements(context["event"].id, DeliveryExp).order_by("order").select_related("system"))
...
def orga_exp_abilities(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    abilities = list(get_elements(context["event"].id, AbilityExp).order_by("order").select_related("typ", "system"))
...
def orga_exp_ability_types(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    context["list"] = get_elements(context["event"].id, AbilityTypeExp).order_by("order")
...
def orga_exp_rules(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    rules = list(get_elements(context["event"].id, RuleExp).order_by("order"))
...
def orga_exp_ability_templates(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    context["list"] = get_elements(context["event"].id, AbilityTemplateExp).order_by("order")
...
def orga_exp_modifiers(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    modifiers = list(get_elements(context["event"].id, ModifierExp).order_by("order"))
...
def orga_exp_criterions(request: HttpRequest, event_slug: str) -> HttpResponse:
    ...
    criterions = list(get_elements(context["event"].id, CriterionExp).order_by("order").select_related("system"))
...
def orga_character_search(request: HttpRequest, event_slug: str) -> JsonResponse:
    ...
    qs = get_elements(context["event"].id, Character).only("id", "uuid", "name", "number")
...
def orga_exp_available(request: HttpRequest, event_slug: str) -> JsonResponse | Http404:
    ...
    if kind == "delivery":
        queryset = get_elements(context["event"].id, DeliveryExp).order_by("number")
        if filter_context == "character" and edit_uuid:
            try:
                character = get_elements(context["event"].id, Character).get(uuid=edit_uuid)
                taken = character.exp_delivery_list.values_list("id", flat=True)
                queryset = queryset.exclude(pk__in=taken)
            except ObjectDoesNotExist:
                return JsonResponse({"res": "ko"})
    else:
        queryset = get_elements(context["event"].id, AbilityExp).order_by("number")
        if filter_context == "character" and edit_uuid:
            try:
                character = get_elements(context["event"].id, Character).get(uuid=edit_uuid)
            ...
            with contextlib.suppress(ObjectDoesNotExist):
                ability = get_elements(context["event"].id, AbilityExp).get(uuid=edit_uuid)
