...
def _exe_users_actions(request: HttpRequest, context: dict, enabled_features) -> None:
    ...
    if "vote" in enabled_features and not get_association_config(context["association_id"], "vote_candidates", default_value="", context=context):
        _add_priority(
            context,
...
def _orga_casting_actions(context, enabled_features) -> None:
    ...
    """
    if "casting" in enabled_features and not get_event_config(context["event"].id, "casting_min", default_value=0, context=context):
        _add_priority(
            context,
...
def _orga_reg_acc_actions(context: dict, enabled_features: list[str]) -> None:
    ...
    if "reduced" in enabled_features and not get_event_config(context["event"].id, "reduced_ratio", default_value=0, context=context):
        _add_priority(
            context,
