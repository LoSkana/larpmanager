...
def casting_characters(context: dict, registration: Registration) -> None:
    ...
    context["faction_filter"] = get_elements(context["event"].id, Faction).filter(typ=FactionType.TRASV)
...
def casting_history_characters(context: dict) -> None:
    ...
    for character in get_elements(context["event"].id, Character).filter(hide=False).select_related("mirror"):
