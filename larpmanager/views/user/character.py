...
def _update_character(context: dict, character: Character, form: Form, message: str, request: HttpRequest) -> str:
    ...
    if get_event_config(context["event"].id, "user_character_approval", default_value=False, context=context) and character.status in [CharacterStatus.CREATION, CharacterStatus.REVIEW] and form.cleaned_data["propose"]:
